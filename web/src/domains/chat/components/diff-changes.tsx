...
export function DiffChanges({ additions, deletions, variant = "default" }: DiffChangesProps) {
  if (variant === "bars") {
...
function DiffBars({ additions, deletions }: { additions: number; deletions: number }) {
  ...
  return (
    ...
    >
      <title>{`${additions} additions, ${deletions} deletions`}</title>
      {Array.from({ length: BLOCK_COUNT }, (_, blockIdx) => {
        const isGreen = blockIdx < greenBlocks;
        return (
          <rect
            key={isGreen ? `add-${blockIdx}` : `del-${blockIdx}`}
            x={blockIdx * 6}
            y={0}
            width={2}
            height={12}
