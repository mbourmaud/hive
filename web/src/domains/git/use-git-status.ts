...
export function useGitStatus(projectPath: string | null) {
  return useQuery({
    ...
    queryFn: () => {
      ...
      return apiClient.get<GitStatus>(`/api/git/status?project_path=${encodeURIComponent(projectPath)}`);
    },
...
export function useFileDiff(projectPath: string | null, filePath: string | null, staged: boolean) {
  return useQuery({
    ...
    queryFn: () => {
      if (projectPath === null || filePath === null) throw new Error("Cannot fetch diff without paths");
