...
function lineColorClass(line: string): string {
  ...
  if (lower.startsWith("[done]") || lower.includes("success") || lower.includes("completed"))
    return "text-success";
      ...
      >
        {logs.length === 0 ? (
          ...
          logs.map((line, lineIdx) => (
            <div
              // biome-ignore lint/suspicious/noArrayIndexKey: log lines are append-only, no reordering
              key={lineIdx}
              className={cn("flex hover:bg-muted/30 transition-colors", lineColorClass(line))}
            >
              <span
              ...
              <span
                className={cn(
                  ...
                  !lineColorClass(line) && "text-muted-foreground",
                )}
              >
                {line}
              </span>
