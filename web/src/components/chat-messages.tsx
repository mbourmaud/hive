...
export function ChatMessages({ messages, members }: ChatMessagesProps) {
  if (!messages || !messages.length) {
    return (
      <div className="text-sm italic py-3 text-muted-foreground">
        ...
        return (
          <div key={i} className="flex gap-2.5 items-start py-2">
            <Avatar className="h-6 w-6 shrink-0">
              <AvatarFallback className="text-[10px] font-bold text-white" style={{ background: fromColor }}>
                {initials}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0">
              <div className="flex items-baseline gap-2 mb-0.5">
                <span className="text-[13px] font-bold" style={{ color: fromColor }}>@{msg.from}</span>
                <ChevronRight className="w-3 h-3 text-muted-foreground shrink-0" />
                <span className="text-xs font-medium" style={{ color: toColor }}>@{msg.to}</span>
