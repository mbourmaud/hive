      ...
      {tasks.map((task) => {
        ...
        return (
          <div key={task.id} className="flex items-start gap-3 py-1.5">
            <div className={`mt-0.5 w-5 text-center text-sm font-bold shrink-0 ${statusColor(task.status)}`}>
              {statusIcon(task.status)}
            </div>
            <div className="flex-1 min-w-0">
              <div className={`text-sm text-foreground ${task.status === "in_progress" ? "font-semibold" : "opacity-70"}`}>
                {task.subject}
              </div>
