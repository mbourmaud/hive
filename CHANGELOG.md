...
- Refactored terminology: PRD to plan, story to task throughout codebase

### Fixed
- Monitor bugs and stability improvements
- Plan approval flow enhancements

## [2.6.1] - 2026-01-25

### Changed
- **Gitignore**: Add `.claude/` directory to gitignore

### Fixed
- **Documentation**: Update obsolete `hive status` references to `hive monitor`
...
### Fixed
- **Monitor UI**: Reduced excessive spacing between status icon and drone name in TUI
...
### Changed
- **Improved Release Pipeline**: More reliable binary downloads across platforms
  - Use `cross` tool with musl targets for portable static Linux binaries
  - Use native macOS runners (Intel + Apple Silicon) for faster builds
  - Add SHA256 checksum verification to install script
  - Update GitHub Actions to latest versions
  - Binary stripping for smaller downloads

### Fixed
- Homebrew formula now correctly finds binary in archive
