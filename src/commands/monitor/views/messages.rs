...
use super::message_parsing::{agent_color, parse_message_text, DisplayMessage};
...
pub(crate) fn render_messages_view(
    ...
    // Load teammates and collect messages
    ...
    let mut messages = collect_messages(&inboxes, &teammates);
    messages.sort_by(|a, b| a.timestamp.cmp(&b.timestamp));

    // Build agent color index
    let agent_index = build_agent_index(&messages);
    ...
    for (msg_idx, msg) in messages.iter().enumerate() {
        ...
        render_message_body(&mut lines, msg, from_color, max_width, &left_marker);
...
fn collect_messages(
    inboxes: &HashMap<String, Vec<task_sync::InboxMessage>>,
    teammates: &HashSet<String>,
) -> Vec<DisplayMessage> {
...
fn build_agent_index(messages: &[DisplayMessage]) -> HashMap<String, usize> {
...
fn render_message_header<'a>(
    lines: &mut Vec<Line<'a>>,
    msg: &DisplayMessage,
    last_from: &Option<String>,
    from_color: Color,
    time_str: &str,
    left_marker: &dyn Fn() -> Span<'a>,
) {
