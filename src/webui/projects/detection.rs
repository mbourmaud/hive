...
use super::types::{DetectionEvent, GitContext, PrInfo, ProjectContext, RuntimeInfo};

// ── Command runner with timeout ─────────────────────────────────────────────

async fn run_cmd(cmd: &str, args: &[&str], cwd: &Path, timeout: Duration) -> Option<String> {
...
pub async fn detect_with_events(path: &Path, tx: mpsc::Sender<DetectionEvent>) {
    ...
    let _ = tx
        .send(DetectionEvent::StepStarted {
            step: "git".to_string(),
            label: "Scanning git repository".to_string(),
        })
        .await;
    ...
    match &git {
        Some(ctx) => {
            ...
            let _ = tx
                .send(DetectionEvent::StepCompleted {
                    step: "git".to_string(),
                    label: "Scanning git repository".to_string(),
                    result,
                })
        ...
        None => {
            let _ = tx
                .send(DetectionEvent::StepFailed {
                    step: "git".to_string(),
                    label: "Scanning git repository".to_string(),
    ...
    let _ = tx
        .send(DetectionEvent::StepStarted {
            step: "runtimes".to_string(),
            label: "Detecting runtimes & versions".to_string(),
        })
        .await;
    ...
    let _ = tx
        .send(DetectionEvent::StepCompleted {
            step: "runtimes".to_string(),
            label: "Detecting runtimes & versions".to_string(),
            result: serde_json::json!(runtime_summary),
        })
        .await;
    ...
    let _ = tx
        .send(DetectionEvent::StepStarted {
            step: "key_files".to_string(),
            label: "Finding configuration files".to_string(),
        })
        .await;
    ...
    let _ = tx
        .send(DetectionEvent::StepCompleted {
            step: "key_files".to_string(),
            label: "Finding configuration files".to_string(),
            result: serde_json::json!(key_files),
        })
        .await;
    ...
    let _ = tx
        .send(DetectionEvent::StepStarted {
            step: "pr".to_string(),
            label: "Checking for open PR/MR".to_string(),
        })
        .await;
